...
\title{Ett bibelstudium om\\Herrens dag}
...
<?php $chapter = 0;?>
\section[<?php 
    echo $t->book($book, true).' '.trim($section['reference']); 
    ?>]{\textmd{{\normalsize <?php 
    echo $section['header']; 
    ?>}\\{\scriptsize <?php
    echo $t->book($book).' '.trim(substr($section['reference'], strrpos($section['reference'], ',')+1));
    ?>}}}
\begin{multicols}{2}
